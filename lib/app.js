...
var pong = {
        ...
        pong.checkChallenge(challenged, function(y2) {
          if (y2 === false) {
            ...
            c.save( function(err, nc) {
              ...
              message = challenger + " has challenged " + challenged + " to a game of chess!";
              console.log(nc);
              cb(message);
            });
          } else {
                ...
                pong.checkChallenge(c4, function(y) {
                  if (y === false) {
                      ...
                      c.save( function(err, nc) {
                        ...
                        message = c1 + " and " + c2 + " have challenged " + c3 + " and " + c4 + " to a game of chess!";
                        cb(message);
                      });
                  } else {
  ...
  getDuelGif: function(cb) {
    ...
    var j = gifs.length - 1;
    for(var i = j; i > -1; i--) {
      j = Math.floor(Math.random() * (_.min([i, j]) + 1));
      var swap = gifs[j];
      gifs[j] = gifs[i];
      gifs[i] = swap;
    }
